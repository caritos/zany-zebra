-- Migration: Robust get_club_members_with_ratings function
-- Date: 2025-09-25
-- Description: Create a robust function that handles missing columns gracefully

-- Drop the existing function completely
DROP FUNCTION IF EXISTS get_club_members_with_ratings(BIGINT);

-- Create a robust function that works even if some columns don't exist
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(profiles.username, split_part(COALESCE(profiles.email, auth_users.email), '@', 1)),
    profiles.nickname,
