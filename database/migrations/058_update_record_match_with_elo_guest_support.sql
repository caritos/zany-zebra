...
BEGIN
  ...
  BEGIN
    ...
    INSERT INTO public.match_records (
      ...
      p_winner, p_notes, auth.uid(), p_match_date
...
-- Add helpful comment
COMMENT ON FUNCTION record_match_with_elo(BIGINT, TEXT, UUID, TEXT, UUID, TEXT, UUID, TEXT, UUID, TEXT, INTEGER, JSONB, TEXT, DATE) IS 'Records tennis match with match sets and ELO rating updates for singles including matches with guest players';