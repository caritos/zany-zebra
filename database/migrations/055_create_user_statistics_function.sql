-- Migration: Create user statistics function
-- Date: 2025-09-26
-- Description: Create function to get comprehensive user match statistics
...
AS $$
  ...
  SELECT
    ...
    COALESCE(COUNT(ms.*)::INTEGER, 0) as total_matches,
    COALESCE(SUM(ms.is_win)::INTEGER, 0) as total_wins,
    COALESCE(COUNT(ms.*) - SUM(ms.is_win), 0)::INTEGER as total_losses,
    CASE
      WHEN COUNT(ms.*) > 0 THEN ROUND((SUM(ms.is_win)::NUMERIC / COUNT(ms.*)) * 100, 1)
      ELSE 0::NUMERIC
    END as win_rate,
    ...
    -- Set statistics
    COALESCE(SUM(ss.user_won_set)::INTEGER, 0) as total_sets_won,
    COALESCE(COUNT(ss.*) - SUM(ss.user_won_set), 0)::INTEGER as total_sets_lost,

    -- Game statistics
    COALESCE(SUM(ss.user_games)::INTEGER, 0) as total_games_won,
    COALESCE(SUM(ss.opponent_games)::INTEGER, 0) as total_games_lost,
  ...
  FROM match_stats ms
  LEFT JOIN set_stats ss ON ss.match_id = ms.id
...
-- Add comment
COMMENT ON FUNCTION get_user_statistics(UUID) IS 'Gets comprehensive match statistics for a user including win/loss records, set/game stats, and ELO ratings';

DO $$
BEGIN
  RAISE NOTICE 'User statistics function created successfully!';
  RAISE NOTICE 'This function provides comprehensive match statistics including singles/doubles breakdown.';
END;
$$;